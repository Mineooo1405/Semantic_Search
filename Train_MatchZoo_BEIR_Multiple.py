...
# Import hàm chunking từ Semantic_Grouping (giả sử đã tạo hàm helper như trước)
# Hoặc import trực tiếp các hàm con nếu muốn tái cấu trúc Semantic_Grouping.py
try:
...
# Import hàm chunking từ Text_Splitter
try:
...
def get_input(prompt: str, default: Optional[str] = None, required: bool = False) -> str:
    """Gets user input with a default value and requirement check."""
    while True:
        ...
        else: # Not required, no default, empty input -> return empty string
             return ""
...
def get_int_input(prompt: str, default: Optional[int] = None, required: bool = False, min_val: Optional[int] = None, max_val: Optional[int] = None) -> Optional[int]:
    ...
    while True:
        ...
        if not response_str and not required:
            return None
        if not response_str and required and default is not None:
             response_str = str(default) # Use default if required and empty
        ...
        except TypeError: # Handle case where response_str is None (shouldn't happen with get_input logic)
...
def get_auto_or_float_input(prompt: str, default: Union[str, float] = 'auto', required: bool = True) -> Union[str, float]:
    """Gets input that can be 'auto' or a float."""
    default_str = str(default)
    while True:
        response_str = get_input(prompt, default_str, required)
        if response_str.lower() == 'auto':
            return 'auto'
        try:
            value = float(response_str)
            # Add range checks if needed, e.g., for thresholds 0.0 to 1.0
            if 0.0 <= value <= 1.0:
                 return value
            else:
                 print("Float value should typically be between 0.0 and 1.0 for thresholds.")
        except ValueError:
            print("Invalid input. Please enter 'auto' or a number (e.g., 0.8).")
        except TypeError:
             if not required: return 'auto' # Default to auto if error and not required
             print("Input error. Please try again.")


def select_chunking_method() -> str:
    """Prompts user to select a chunking method."""
...
def get_common_params() -> Dict:
    """Gets common configuration parameters from the user."""
    ...
    params['max_docs'] = max_docs if max_docs is not None and max_docs > 0 else None # Store None if 0 or empty
    # --------------------
    return params
...
def get_semantic_splitter_params(embedding_model: str) -> Dict:
    """Gets parameters specific to Semantic Splitter."""
...
def get_text_splitter_params() -> Dict:
    """Gets parameters specific to Text Splitter."""
...
if selected_method == 'semantic_grouping':
    ...
    # Add model_name even if not used by chunking, for consistency if needed elsewhere
...
EMBEDDING_MODEL_NAME = common_config['embedding_model'] # Keep this separate for clarity
...
# Parameters are already in method_params dictionary
...
# --- Main Processing ---
# (Main logic remains the same, but uses variables set by UI instead of args)
...
data_folder_to_load = LOCAL_DATA_PATH # Path to check initially
...
try:
    ...
    try:
        ...
        # Target directory for download is the PARENT of LOCAL_DATA_PATH
        download_target_dir = os.path.dirname(LOCAL_DATA_PATH)
        if not download_target_dir: download_target_dir = "." # Handle case if LOCAL_DATA_PATH is just "msmarco"

        util.download_and_unzip(url, download_target_dir)
        print(f"Dataset downloaded and unzipped potentially into: {LOCAL_DATA_PATH}")

        # --- ADDED CHECK: Adjust path if subfolder exists ---
        ...
        # Try loading again with the potentially adjusted path
        corpus, queries, qrels = GenericDataLoader(data_folder=data_folder_to_load).load(split="train")
...
if not os.path.exists(CHUNKS_OUTPUT_PATH) or not os.path.exists(DOC_CHUNK_MAP_PATH):
    ...
    doc_to_chunk_map = {}
    processed_docs = 0
    docs_processed_count = 0 # Counter for the loop limit

    try:
        # --- ADJUSTED LOOP ---
        # Use tqdm with total if limit is set, otherwise iterate normally
        corpus_iterable = corpus.items()
        if MAX_DOCS_TO_PROCESS:
            corpus_iterable = list(corpus.items())[:MAX_DOCS_TO_PROCESS] # Take only the first N items
            tqdm_total = MAX_DOCS_TO_PROCESS
        else:
            tqdm_total = len(corpus) # Use full length if no limit
        ...
        with open(CHUNKS_OUTPUT_PATH, 'w', encoding='utf-8') as f_chunks_out:
            for doc_id, doc_data in tqdm(corpus_iterable, desc=f"Chunking Corpus ({selected_method})", total=tqdm_total):
                # --- Limit check (redundant if corpus_iterable is sliced, but safe) ---
                # if MAX_DOCS_TO_PROCESS and docs_processed_count >= MAX_DOCS_TO_PROCESS:
                #    break
                # --------------------------------------------------------------------
                ...
                processed_docs += 1
                docs_processed_count += 1 # Increment counter
    ...
    # Keep chunk_to_doc_map for triplet generation if needed, or del later
...
# (Triplet generation logic remains the same)
...
# 3. Load Chunks Data (!!! MEMORY WARNING !!!)
...
try:
    with open(TRAIN_TRIPLETS_PATH, 'w', encoding='utf-8') as f_train_out:
        ...
        for qid in tqdm(query_ids, desc="Generating Triplets"):
            ...
            # Create one triplet per positive chunk (simple approach)
            for positive_chunk_id in positive_chunk_ids_for_query:
                ...
                for _ in range(10): # Try 10 times to find a negative
                ...
                if negative_chunk_id:
                    ...
                    if negative_chunk_text:
                        # Write to file in MatchZoo format (tab separated)
                        ...
                        if MAX_TRIPLETS_TO_GENERATE is not None and triplets_count >= MAX_TRIPLETS_TO_GENERATE:
                            ...
                            raise StopIteration # Stop outer loop

            if MAX_TRIPLETS_TO_GENERATE is not None and triplets_count >= MAX_TRIPLETS_TO_GENERATE:
                 break # Stop completely if limit reached

except StopIteration:
    pass # Catch StopIteration to end gracefully when limit is reached
except Exception as e:
